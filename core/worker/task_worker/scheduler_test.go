...
func TestClearExpiredRuntimes(t *testing.T) {
	...
	store.SetTaskRuntime(validRuntime)
	store.SetTaskRuntime(&definition.TaskRuntime{
		Id:            "r0",
...
func TestGetCurrentAssignments(t *testing.T) {
	...
	store.SetTaskAssignment(&definition.TaskAssignment{
		TaskId:    TEST_TASK_ID,
		ItemId:    "p1",
		RuntimeId: w.runtime.Id,
	})
	assignments1, _ := store.GetTaskAssignments(TEST_TASK_ID)
	assert.Equal(t, 1, len(assignments1))

	assignMap, spares, runtimeAssigns, _ := w.getCurrentAssignments()

	assignments2, _ := store.GetTaskAssignments(TEST_TASK_ID)
...
func TestDistributeTaskItems(t *testing.T) {
	...
	assert.True(t, store.ShouldTaskReloadItems(TEST_TASK_ID, w.runtime.Id))
}
...
func TestReloadTaskItems(t *testing.T) {
	...
	assign, _ := store.GetTaskAssignment(TEST_TASK_ID, TEST_ITEM_ID1)
	assign.RequestedRuntimeId = "r1"
	store.SetTaskAssignment(assign)

	w.reloadTaskItems()

	assert.Equal(t, 1, len(w.taskItems))
	assert.False(t, store.ShouldTaskReloadItems(TEST_TASK_ID, w.runtime.Id))
	assert.True(t, store.ShouldTaskReloadItems(TEST_TASK_ID, "r1"))
}
...
func TestSchedule(t *testing.T) {
	...
	r, _ := store.GetTaskAssignment(TEST_TASK_ID, TEST_ITEM_ID1)
	assert.NotNil(t, r)
